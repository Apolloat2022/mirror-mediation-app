  ...
  const audioClips = [
    {
      ...
      description: "A guided contemplation from the Mirror",
      duration: "2:45",
  ...
  return (
    <LinearGradient colors={['#0A0A0F', '#1A1A2E']} style={styles.gradient}>
      <ScrollView contentContainerStyle={styles.container}>
        ...
        {audioClips.map((clip) => (
          <View key={clip.id} style={styles.audioCard}>
            <View style={styles.audioInfo}>
              <Text style={styles.audioTitle}>{clip.title}</Text>
              <Text style={styles.audioDescription}>{clip.description}</Text>
        ...
        <Text style={styles.note}>
          • Listen to contemplative guidance{'\n'}
